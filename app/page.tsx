  ...
  return (
    <main className="relative min-h-screen bg-black text-white overflow-hidden font-sans">
      ...
      <header className="absolute top-0 left-0 w-full px-8 py-6 flex justify-between items-center z-10">
        ...
        <Link href="#" className="text-sm bg-white text-black px-4 py-1.5 rounded-full hover:bg-white/90 transition">Create Account</Link>
      </header>

      {/* Hero Section */}
      <section className="flex flex-col justify-center items-center text-center min-h-screen px-6 z-10 relative">
        <p className="text-xs uppercase tracking-widest text-white/50 mb-4">Unlock Your Assets Spark!</p>
        <h1 className="text-5xl md:text-6xl font-extrabold tracking-tight leading-tight mb-6">
          One-click for <span className="text-white/60">Asset Defense</span>
        </h1>
        <p className="text-white/60 max-w-xl text-base md:text-lg mb-10">
          Dive into the art assets, where innovative blockchain tech meets financial expertise.
        </p>

        <div className="flex gap-4 mb-16">
          <Link href="/market">
            <button className="bg-white text-black px-6 py-3 rounded-full font-medium text-sm hover:bg-white/90 transition-all shadow-md">
              Discover More
            </button>
          </Link>
          <Link href="#">
            <button className="bg-white/10 border border-white/20 px-6 py-3 rounded-full font-medium text-sm hover:bg-white/5 transition-all">
              Open App
            </button>
          </Link>
        </div>
